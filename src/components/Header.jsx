  ...
  return (
    <div className="sticky top-0 z-20">
      <header className="Encabezado h-12 text-2xl 2xl:h-24 2xl:text-4xl xl:h-20 xl:text-3xl lg:h-18 lg:text-2xl md:h-14 md:text-3xl">
        <button className="mx-4 my-1 w-24 z-40">
          {/* Boton de inicio, regresará hasta arriba */}
          <img id="TopLogo" src={logo} className="Logo" alt="Logo del Salón Colibrí"></img>
        </button>

        {componentes.map((comp) => {
          return <button className="Rm Titulo z-40">{comp.nombre}</button>;
        })}

        <button
          name="Hamburguesa"
          id="menu"
          className="Titulo absolute inset-y-0 right-0 text-4xl z-40"
        ...
        <div className='fixed top-0 left-0 w-full h-full z-0' onClick={cerrarMenu}></div> 
        {/* El div superior se encarga de cerrar el menú hamburguesa cuando ya está desplegado */}
        <div
          id="DivHamb"
